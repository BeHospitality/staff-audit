...
export default function OrgDetailView({ orgId, orgName, orgCode, onBack, onGenerateDossier }: Props) {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => setResponses((data as PulseResponse[]) || []));
  }, [orgId]);
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">{filtered.length} responses</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" size="sm" onClick={handleExport}><Download className="w-4 h-4 mr-1" /> PDF</Button>
            <Button variant="gold" size="sm" onClick={onGenerateDossier}><FileText className="w-4 h-4 mr-1" /> Dossier</Button>
          </div>
        </div>

        {/* Survey Link */}
        <div className="bg-primary/10 border border-primary/30 rounded-lg p-4 flex flex-col md:flex-row md:items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={handleWhatsApp}><MessageCircle className="w-4 h-4" /></Button>
        </div>

        {/* Overall Health */}
        <div className={`rounded-lg border p-6 text-center ${scoreBg(overall)}`}>
